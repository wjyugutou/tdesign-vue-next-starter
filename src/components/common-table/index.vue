...
function handleClickDelete(slot: { row: { rowIndex: number } }) {
  deleteIdx.value = slot.row.rowIndex
  confirmVisible.value = true
}
function onReset(val: unknown) {
  console.log(val)
}
...
function onSubmit(val: unknown) {
  console.log(val)
  console.log(formData.value)
}
function rehandlePageChange(pageInfo: PageInfo, newDataSource: TableRowData[]) {
...
<template>
  <div class="list-common-table">
    <TForm :data="formData" :label-width="80" colon @reset="onReset" @submit="onSubmit">
      <TRow>
        <TCol :span="10">
          <TRow :gutter="[24, 24]">
            <TCol :span="4">
