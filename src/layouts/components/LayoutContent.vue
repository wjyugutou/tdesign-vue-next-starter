...
function renderTitle(title: string | Record<string, string>) {
  if (typeof title === 'string')
    return title
  return title.zh_CN || title.en_US || ''
}
...
<template>
  <TLayout :class="`${prefix}-layout`">
      ...
      >
        <template #label>
          ...
          >
            ...
            <TIcon v-else name="home" />
            <template #dropdown>
              <TDropdownMenu>
                <TDropdownItem @click="() => handleRefresh(routeItem, index)">
                  <TIcon name="refresh" />
                  刷新
                </TDropdownItem>
                <TDropdownItem v-if="index > 1" @click="() => handleCloseAhead(routeItem.path, index)">
                ...
                >
                  <TIcon name="arrow-right" />
                  关闭右侧
                </TDropdownItem>
                <TDropdownItem v-if="tabRouters.length > 2" @click="() => handleCloseOther(routeItem.path, index)">
                  <TIcon name="close-circle" />
                  关闭其他
                </TDropdownItem>
              </TDropdownMenu>
            </template>
    ...
    <TContent :class="`${prefix}-content-layout`">
      <LBreadcrumb v-if="settingStore.showBreadcrumb" />
      <LContent />
    </TContent>
    <TFooter v-if="settingStore.showFooter" :class="`${prefix}-footer-layout`">
      <LFooter />
    </TFooter>
  </TLayout>
</template>
